...
#define TGT_REG0_DDR  			0x00000014
#define TGT_REG1_DDR   			0x00000018
#define TGT_REG2_DDR  			0x0000001c
...
static long srio_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd){
		...
		case SRIO_DOORBELL_DATE:
			...
			doorbell_date = (unsigned int)((*tgt_reg0>>8) & 0xffff);
			
			printk("kennel-(*tgt_reg0 & 0xffff) = %x\n",(*tgt_reg0 & 0xffff));
			
			//将size传输到应用层，给target使用
			ret = copy_to_user((unsigned char *)arg, &doorbell_date, 16); //copy_to_user();
			printk("srio_ioctl_nread-ret = -%d-\n",ret);
