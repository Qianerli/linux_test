...
volatile unsigned short *ram_sta_addr			= NULL;

volatile unsigned short *ram_pdate_addr			= NULL;
...
static int __init dev1553b_init(void)
{
	...
	//4.申请dev1553b0资源，映射
    ram_cfg_addr = (volatile unsigned short *)ioremap(RAM_DDR, 16);
	ram_sta_addr = (volatile unsigned short *)ioremap(RAM_DDR+STATUS_REG_DDR, 16);
	ram_pdate_addr = (volatile unsigned short *)ioremap(RAM_DDR+SHORTMASK_REG_DDR, 16);
	
	ram_pdate1 = (volatile unsigned short *)ioremap(RAM_DDR+0x00002000, 16);
